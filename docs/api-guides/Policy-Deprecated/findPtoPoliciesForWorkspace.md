...
| `page-size` | query | string | No |  | Page size. |
| `name` | query | string | No |  | If provided, you'll get a filtered list of policies that contain the provided string in their name. |
| `status` | query | string | No |  | If provided, you'll get a filtered list of policies with the corresponding status. |
...
### Code examples
```bash
curl -X GET "https://developer.clockify.me/v1/workspaces/{workspaceId}/policies" \
...
- Artifact: `scripts/api-docs-runner/artifacts/findPtoPoliciesForWorkspace-200.json`
```json
[
  {
