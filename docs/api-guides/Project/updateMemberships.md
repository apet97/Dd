...
- **Required:** Yes
- **Schema summary:**
- `memberships` (array, required) Represents a list of users with id and rates request objects.
...
### Code examples
```bash
curl -X PATCH "https://developer.clockify.me/v1/workspaces/{workspaceId}/projects/{projectId}/memberships" \
  -H 'X-Api-Key: <YOUR_API_KEY>' \
  -H 'Accept: application/json' \
  -H 'Content-Type: application/json'
  -d '{"memberships":[{"$ref":"#/components/schemas/UserIdWithRatesRequest"}],"userGroups":{"$ref":"#/components/schemas/UserGroupIdsSchema"}}'
```
...
- Artifact: `scripts/api-docs-runner/artifacts/updateMemberships-200.json`
```json
{
  "archived": false,
  "billable": false,
