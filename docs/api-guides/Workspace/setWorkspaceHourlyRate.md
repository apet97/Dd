...
### Code examples
```bash
curl -X PUT "https://developer.clockify.me/v1/workspaces/{workspaceId}/hourly-rate" \
  -H 'X-Api-Key: <YOUR_API_KEY>' \
  -H 'Accept: application/json' \
  -H 'Content-Type: application/json'
  -d '{"amount":2000,"currency":"USD","since":"2020-01-01T00:00:00Z"}'
```

```javascript
import fetch from 'node-fetch';

const response = await fetch('https://developer.clockify.me/v1/workspaces/{workspaceId}/hourly-rate', {
  method: 'PUT',
  headers: {
...
url = 'https://developer.clockify.me/v1/workspaces/{workspaceId}/hourly-rate'
headers = {
...
- Artifact: `scripts/api-docs-runner/artifacts/setWorkspaceHourlyRate-200.json`
```json
{
