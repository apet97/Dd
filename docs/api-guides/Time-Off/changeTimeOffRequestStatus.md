...
- `status` (string, optional) Provide the status you would like to use for changing the time off request.
- **Minimal example:**
```json
{
...
### Code examples
```bash
curl -X PATCH "https://developer.clockify.me/v1/workspaces/{workspaceId}/time-off/policies/{policyId}/requests/{requestId}" \
  -H 'X-Api-Key: <YOUR_API_KEY>' \
  -H 'Accept: application/json' \
  -H 'Content-Type: application/json'
  -d '{"note":"Time Off Request Note","status":"APPROVED"}'
```

```javascript
import fetch from 'node-fetch';

const response = await fetch('https://developer.clockify.me/v1/workspaces/{workspaceId}/time-off/policies/{policyId}/requests/{requestId}', {
  method: 'PATCH',
  headers: {
...
url = 'https://developer.clockify.me/v1/workspaces/{workspaceId}/time-off/policies/{policyId}/requests/{requestId}'
headers = {
...
- Artifact: `scripts/api-docs-runner/artifacts/changeTimeOffRequestStatus-200.json`
```json
{
  "balanceDiff": 1,
