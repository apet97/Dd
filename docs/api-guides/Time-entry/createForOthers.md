...
| Name | In | Type | Required | Default | Description |
|------|----|------|----------|---------|-------------|
| `from-entry` | query | string | No |  | Represents time entry identifier across the system. |
...
### Code examples
```bash
curl -X POST "https://developer.clockify.me/v1/workspaces/{workspaceId}/user/{userId}/time-entries" \
...
- Artifact: `scripts/api-docs-runner/artifacts/createForOthers-201.json`
```json
{
  "billable": false,
  "customFieldValues": [
