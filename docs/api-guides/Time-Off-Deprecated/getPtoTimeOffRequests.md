## POST /v1/workspaces/{workspaceId}/requests
...
- `statuses` (string, optional) Filters time off requests by status.
...
### Code examples
```bash
curl -X POST "https://developer.clockify.me/v1/workspaces/{workspaceId}/requests" \
...
const response = await fetch('https://developer.clockify.me/v1/workspaces/{workspaceId}/requests', {
  method: 'POST',
  headers: {
...
url = 'https://developer.clockify.me/v1/workspaces/{workspaceId}/requests'
headers = {
...
- Artifact: `scripts/api-docs-runner/artifacts/getPtoTimeOffRequests-200.json`
```json
{
  "count": 1,
  "requests": [
